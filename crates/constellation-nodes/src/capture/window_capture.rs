...
impl WindowCaptureNode {
    pub fn new(id: Uuid, config: NodeConfig) -> Result<Self> {
        ...
        parameters.insert("window_id".to_string(), ParameterDefinition {
        ...
        parameters.insert("follow_window".to_string(), ParameterDefinition {
        ...
        parameters.insert("capture_method".to_string(), ParameterDefinition {
    ...
    fn initialize_capture(&mut self) -> Result<()> {
        let window_title = self.get_parameter("window_title")
            .and_then(|v| v.as_str().map(|s| s.to_string()))
            .unwrap_or_default();

        let window_id = self.get_parameter("window_id")
            .and_then(|v| v.as_i64())
            .unwrap_or(0) as u64;
        ...
        self.capture_context = if !window_title.is_empty() {
            ...
            return Err(anyhow::anyhow!("Either window_title or window_id must be specified"));
        };

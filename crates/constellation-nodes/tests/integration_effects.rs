...
use constellation_nodes::effects::{ColorCorrectionNode, BlurNode, SharpenNode};
...
fn create_test_frame_data(width: u32, height: u32) -> FrameData {
    FrameData {
        video_data: Some(create_test_video_frame(width, height)),
        audio_data: None,
        tally_data: None,
        scene3d_data: None,
...
fn test_color_correction_node_creation_and_properties() {
    ...
    assert!(node.is_ok(), "Color correction node creation should succeed");
    ...
    assert!(matches!(properties.node_type, NodeType::Effect(EffectType::ColorCorrection)));
    assert_eq!(properties.input_types, vec![ConnectionType::Video]);
    assert_eq!(properties.output_types, vec![ConnectionType::Video]);
...
fn test_color_correction_node_parameter_defaults() {
    ...
    assert!(matches!(brightness_param.parameter_type, ParameterType::Float));
    assert_eq!(brightness_param.default_value, serde_json::Value::from(1.0));
    assert_eq!(brightness_param.min_value, Some(serde_json::Value::from(0.0)));
    assert_eq!(brightness_param.max_value, Some(serde_json::Value::from(3.0)));
...
fn test_color_correction_brightness_adjustment() {
    ...
    config.parameters.insert("brightness".to_string(), serde_json::Value::from(2.0));
    
    let mut node = ColorCorrectionNode::new(node_id, config).unwrap();
    let input_frame = create_test_frame_data(4, 4);
    
    // Get original pixel for comparison
    let original_pixel = input_frame.video_data.as_ref().unwrap().data[0];
...
fn test_blur_node_creation_and_properties() {
    ...
    assert!(matches!(properties.node_type, NodeType::Effect(EffectType::Blur)));
    assert_eq!(properties.input_types, vec![ConnectionType::Video]);
    assert_eq!(properties.output_types, vec![ConnectionType::Video]);
...
fn test_blur_node_processing() {
    ...
    // Set blur radius
    config.parameters.insert("radius".to_string(), serde_json::Value::from(2.0));
...
fn test_blur_node_zero_radius() {
    ...
    config.parameters.insert("radius".to_string(), serde_json::Value::from(0.0));
    
    let mut node = BlurNode::new(node_id, config).unwrap();
    let input_frame = create_test_frame_data(16, 16);
    
    let original_data = input_frame.video_data.as_ref().unwrap().data.clone();
...
fn test_sharpen_node_creation_and_properties() {
    ...
    assert!(matches!(properties.node_type, NodeType::Effect(EffectType::Sharpen)));
    assert_eq!(properties.input_types, vec![ConnectionType::Video]);
    assert_eq!(properties.output_types, vec![ConnectionType::Video]);
    ...
    assert!(matches!(strength_param.parameter_type, ParameterType::Float));
...
fn test_sharpen_node_processing() {
    ...
    config.parameters.insert("strength".to_string(), serde_json::Value::from(1.5));
...
fn test_sharpen_node_zero_strength() {
    ...
    config.parameters.insert("strength".to_string(), serde_json::Value::from(0.0));
...
fn test_effects_chain_processing() {
    ...
    let mut config1 = NodeConfig { parameters: HashMap::new() };
    config1.parameters.insert("brightness".to_string(), serde_json::Value::from(1.2));
    let mut color_node = ColorCorrectionNode::new(node_id1, config1).unwrap();
    
    let mut config2 = NodeConfig { parameters: HashMap::new() };
    config2.parameters.insert("radius".to_string(), serde_json::Value::from(1.0));
    let mut blur_node = BlurNode::new(node_id2, config2).unwrap();
    
    let mut config3 = NodeConfig { parameters: HashMap::new() };
    config3.parameters.insert("strength".to_string(), serde_json::Value::from(0.5));
    ...
    assert!(final_output.video_data.is_some());
    
    let video_frame = final_output.video_data.unwrap();
    assert_eq!(video_frame.width, 64);
    assert_eq!(video_frame.height, 64);
    assert_eq!(video_frame.format, VideoFormat::Rgba8);
}
...
fn test_effects_parameter_updates() {
    ...
    assert!(result.is_ok());
    assert_eq!(node.get_parameter("brightness"), Some(serde_json::Value::from(2.5)));
    
    let result = node.set_parameter("contrast", serde_json::Value::from(1.8));
    assert!(result.is_ok());
    assert_eq!(node.get_parameter("contrast"), Some(serde_json::Value::from(1.8)));
...
fn test_effects_preserve_non_video_data() {
    ...
    let input_frame = FrameData {
        video_data: Some(create_test_video_frame(32, 32)),
    ...
    // Audio and tally should be preserved
    assert!(output.audio_data.is_some());
    assert!(output.tally_data.is_some());
    ...
    let tally = output.tally_data.unwrap();
    assert!(tally.program_tally);
    assert!(!tally.preview_tally);
}
...
fn test_effects_with_no_video_data() {
    ...
    // No video data should remain None
    assert!(output.video_data.is_none());

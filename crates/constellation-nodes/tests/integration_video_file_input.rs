...
use constellation_nodes::video_file::{VideoFileReader, VideoMetadata};
...
fn create_test_video_file(name: &str, extension: &str) -> PathBuf {
    ...
    file.write_all(b"dummy video file content for testing").unwrap();
...
fn test_video_file_input_node_creation_and_properties() {
    ...
    assert!(node.is_ok(), "Video file input node creation should succeed");
    ...
    assert!(matches!(properties.node_type, NodeType::Input(InputType::VideoFile)));
...
fn test_video_file_input_node_parameter_defaults() {
    ...
    assert!(matches!(file_path_param.parameter_type, ParameterType::String));
    assert_eq!(file_path_param.default_value, serde_json::Value::String("".to_string()));
...
fn test_video_file_input_node_without_file_path() {
    ...
    let audio_frame = output.audio_data.unwrap();
...
fn test_video_file_input_node_with_valid_file() {
    ...
    assert_eq!(audio_frame.sample_rate, 48000);
    assert_eq!(audio_frame.channels, 2);
    assert!(!audio_frame.samples.is_empty());
...
fn test_video_file_input_node_with_loop_enabled() {
    ...
    config.parameters.insert("loop".to_string(), serde_json::Value::Bool(true));
    
    let mut node = VideoFileInputNode::new(node_id, config).unwrap();
    
    // Verify parameter was set
    assert_eq!(node.get_parameter("loop"), Some(serde_json::Value::Bool(true)));
...
fn test_video_file_input_node_parameter_updates() {
    ...
    assert_eq!(
        ...
        Some(serde_json::Value::String(test_file.to_string_lossy().to_string()))
    );
    assert_eq!(node.get_parameter("loop"), Some(serde_json::Value::Bool(true)));
...
fn test_video_file_reader_loop_playback() {
    ...
    reader.seek_to_frame(2998).unwrap(); // Near end of 3000 frames
    
    // Read remaining frames
    let _ = reader.read_frame().unwrap(); // Frame 2999
    let _ = reader.read_frame().unwrap(); // Should loop back to frame 0
    
    // Should be back at frame 1 after reading frame 0
    assert_eq!(reader.current_frame(), 1);

...
fn test_camera_input_node_parameter_defaults() {
    ...
    assert!(matches!(
        resolution_param.parameter_type,
        ParameterType::Enum(_)
    ));
    assert_eq!(
        resolution_param.default_value,
...
fn test_camera_input_with_valid_parameters() {
    ...
    config
        .parameters
        .insert("fps".to_string(), serde_json::Value::from(30));
